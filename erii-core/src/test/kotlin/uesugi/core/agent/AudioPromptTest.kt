...
class AudioPromptTest {
    ...
    fun `audio capable prompt attaches original audio bytes`() = runBlocking {
        ...
        val prompt = buildPrompt(
            context = context {
                reads++
                AudioResource(bytes, "mp3", "voice.mp3")
            },
    ...
    fun `audio disabled uses placeholder without reading object storage`() = runBlocking {
        ...
        val prompt = buildPrompt(
            context = context {
                reads++
                AudioResource(byteArrayOf(1), "mp3", "voice.mp3")
            },
            supportsVision = false,
            supportsAudio = false,
        )
    ...
    fun `missing invalid or failed audio falls back to placeholder`() = runBlocking {
        val contexts = listOf(
            context { null },
            context { AudioResource(byteArrayOf(1), "bin", "voice.bin") },
            context { error("storage failed") },
        )
    ...
    private fun context(
        ...
        audio: suspend (HistoryRecord) -> AudioResource?,
    ) = Context(
        currentBotId = "bot-a",
        groupId = "group-a",
        echo = "echo",
